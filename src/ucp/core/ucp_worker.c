...
static ucs_status_t ucp_worker_add_resource_ifaces(ucp_worker_h worker)
{
    ...
    ucs_for_each_bit(tl_id, context->tl_bitmap) {
        if (!(ucp_worker_is_tl_p2p(worker, tl_id) ||
                               ...
                               ucp_worker_h *worker_p)
{
    ...
    /* Initialize extensions registered for this context */
    status = ucp_worker_init_extensions(worker);
    /* @note must be before UCT iface init, so that new AM handlers are used */
    if (status != UCS_OK) {
        goto err_am_cleanup;
    }
