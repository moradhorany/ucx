                              ...
                              void *arg, unsigned flags)
{
    ...
    int is_lock_needed = !is_short && (flags & UCT_SEND_FLAG_PACK_LOCK);
    if (is_lock_needed) {
                       ...
                       uct_mm_coll_fifo_element_t *elem, int is_incast)
{
    ...
    if (ucs_unlikely(status == UCS_INPROGRESS)) {
        ...
        if (is_incast) {
            ...
            uct_mm_coll_ep_is_last_to_read(elem, iface->super.recv_check.fifo_ctl,
                    proc_cnt, iface->super.recv_check.read_index);
        } else {
            /* set information for @ref uct_mm_coll_iface_release_shared_desc */
            uct_recv_desc(desc) = (void*)((uintptr_t)ep->coll_id);
        }
    } else if (is_incast) {
        /* Mark iface element as done (and re-usable) */
        uct_mm_coll_ep_is_last_to_read(elem, iface->super.recv_check.fifo_ctl,
                proc_cnt, iface->super.recv_check.read_index);
    } else {
        /* Mark endpoint element as done (and re-usable) */
        uct_mm_bcast_ep_t *bcoll_ep = ucs_derived_of(ep, uct_mm_bcast_ep_t);
        uct_mm_coll_ep_is_last_to_read(elem, bcoll_ep->recv_check.fifo_ctl,
                proc_cnt, bcoll_ep->recv_check.read_index);
    }
...
{
    int is_loopback;
    *ep_p = uct_mm_coll_ep_check_existing(params, &is_loopback);
    if (*ep_p) {

        return UCS_OK;
    }

    ucs_status_t status = UCS_CLASS_NEW(uct_mm_bcast_ep_t, ep_p, params);
    if (status == UCS_OK) {
        ((uct_mm_coll_ep_t*)*ep_p)->ref_count++;
        if (!is_loopback) {
            uct_mm_coll_ep_add((uct_mm_coll_ep_t*)*ep_p);
        }
    }
...
ucs_status_t uct_mm_incast_ep_create(const uct_ep_params_t *params, uct_ep_h *ep_p)
{
    ...
    *ep_p = uct_mm_coll_ep_check_existing(params, &is_loopback);
    ...
    ucs_status_t status = UCS_CLASS_NEW(uct_mm_incast_ep_t, ep_p, params);
    if (status == UCS_OK) {
        ((uct_mm_coll_ep_t*)*ep_p)->ref_count++;
        if (!is_loopback) {
            uct_mm_coll_ep_add((uct_mm_coll_ep_t*)*ep_p);
                                   ...
                                   uct_mm_bcast_ep_t *ep)
{
    ...
    ep->recv_check.read_elem = UCT_MM_IFACE_GET_FIFO_ELEM(&iface->super.super,
            ep->super.super.fifo_elems, (read_index & ep->super.fifo_mask));

    uct_mm_progress_fifo_tail(&ep->recv_check);
